...
self.addEventListener('fetch', function (event) {
  ...
  //            above (CACHE_NAME)
  let cacheopen = caches.open(CACHE_NAME);
  event.respondWith(
    cacheopen.then((c) => {
      return c.match(event.request).then((cachedResponse) => {
        let fetchresp = fetch(event.request).then((networkResponse) => {
          c.put(event.request, networkResponse.clone());
          return networkResponse;
        });
        return cachedResponse || fetchresp;
      });
    })
  );
